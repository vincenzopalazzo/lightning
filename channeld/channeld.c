...
/* Get the remote node amount after the splice */
static struct amount_msat remote_balance(struct peer *peer,
					    enum tx_role our_role,
					    const struct wally_psbt *psbt,
					    int chan_output_index,
					    int chan_input_index)
{
	s64 relative_remote_val;
	/* Current value of the remote channel balance */
	struct amount_msat push_value;

	/* We calculcate the `push_value` to send to the
	 * hsmd, that is the remote amount in the channel
	 * after the splice. */
	push_value = peer->channel->view[REMOTE].owed[REMOTE];
	switch (our_role) {
	case TX_INITIATOR:
		relative_remote_val = peer->splicing->accepter_relative;
		break;
	case TX_ACCEPTER:
		relative_remote_val = peer->splicing->opener_relative;
		break;
	default:
	...
	if (!amount_msat_add_sat_s64(&push_value, push_value, relative_remote_val))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "Unable to add to the amount %s the"
				 " value %ld",
...
static void splice_accepter(struct peer *peer, const u8 *inmsg)
{
	...
	current_push_val = remote_balance(peer, our_role,ictx->current_psbt,
					  outpoint.n, splice_funding_index);
