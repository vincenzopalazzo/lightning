			...
			enum invoice_status status)
{
	...
	stmt = db_prepare_v2(db, SQL("SELECT id"
				     ...
				     " WHERE state = ?"
				     "   AND expiry_time <= ?"));
	db_bind_int(stmt, 0, status);
	db_bind_u64(stmt, 1, expiry_time);
...
static void trigger_expiration(struct invoices *invoices)
{
	...
	ids = expired_ids(tmpctx, invoices->wallet->db, now, UNPAID);
	...
	for (size_t i = 0; i < tal_count(ids); i++) {
		...
		db_bind_u64(stmt, 1,
			    ...
			    invoice_index_update(invoices->wallet->ld,
						 EXPIRED, NULL, NULL));
		db_bind_u64(stmt, 2, ids[i]);
		db_exec_prepared_v2(take(stmt));

		/* Trigger expiration */
		inv.id = ids[i];
		trigger_invoice_waiter_expire_or_delete(invoices, ids[i], &inv);
	}
			     ...
			     u64 max_expiry_time)
{
	...
	for (size_t i = 0; i < tal_count(ids); i++) {
		...
		inv.id = ids[i];
		details = invoices_get_details(tmpctx, invoices, inv);
		stmt = db_prepare_v2(invoices->wallet->db, SQL(
					     "DELETE FROM invoices"
					     " WHERE id = ?;"));
		db_bind_u64(stmt, 0, ids[i]);
		db_exec_prepared_v2(take(stmt));
